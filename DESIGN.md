...
hero（全屏影像）→ PHOTOGRAPHY（一屏 3D 曲面照片墙，11 帧 + 诗区）→ THE ARCHIVE（六 tab：书 6 / 影 16 / 剧 19 / 音乐 763 首 16 组 / 球队 5 / 游戏 18）→ ABOUT（统计）→ footer。

- 照片 / 影 / 剧 / 游戏 / 音乐共用唯一的 `#lightbox`。
- 照片区是**一屏 3D 曲面墙**（复刻 cmscurvegallery.framer.website，实测值见下）：11 张瓦片贴在以观看者为球心的球面上，拖拽横向无限循环。几何与三个坑见 `AGENTS.md`「站点结构」。
- 瓦片**只放横构图 plate**（11 张全部是 1600×1067 = 3:2），瓦片就是 3:2，`object-fit: cover` 因此**零裁切**。**绝不竖裁**：参考站把每张裁成 4:5，等于切掉约 47% 宽度。
- 实测（本机 headless Chrome，1418×802）：R = 928.7px、瓦片 264×176、列缝 3–5px、行缝约 9px、外围瓦片宽度 376px = 中心的 **1.42×**、可见瓦片 25 张、整墙 154 张。390×844 时 `MAX_ROWS` 刹车生效：瓦片 199×132、R = 696.8px、整墙仍 154 张。
